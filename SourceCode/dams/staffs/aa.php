...
$data = mysqli_query($connection, "SELECT * FROM data_start WHERE college = 'CICS' AND campus = 'ARASOF' AND academic_year_start = '2023' AND academic_year_end = '2024' ORDER BY last_name ");
...
while ($item = mysqli_fetch_array($data)) {
    ...
    $cellValueNew = $spreadsheet->getActiveSheet()->getCell('A' . $currentContentRow)->getValue();
      
            // $cellValueNew = $currentCellValue;
        if ($cellValueNew == $cellValueOld) {
            $spreadsheet->getActiveSheet()->mergeCells('A' . $currentContentRow . ':C' . ($currentContentRow - 1));
        } 

        else{
            $spreadsheet->getActiveSheet()->mergeCells('A'.$currentContentRow.':C'.($currentContentRow));
...
$spreadsheet->getActiveSheet()->removeRow($currentContentRow, 4);
...
$contentStartRow = $currentContentRow + 2;
...
while ($item = mysqli_fetch_array($data)) {
    if ($item['type'] === 'temporary') {
        ...
        // Fill the cell with data
        $spreadsheet->getActiveSheet()
            ->setCellValue('A' . $currentContentRow_temp, $item['first_name'] . ' ' . $item['last_name'])
...
$contentStartRow = $currentContentRow_temp + 1;
