...
$spreadsheet = $reader->load("../../../templates/ipcr_faculty.xlsx");
...
$start_of_term_new = ob_get_clean();
$type =  $_GET['type'];
$type_designation = "faculty member";
//if faculty just a faculty member then dont change/ if head or dean then it must be head or dean
if($type == 'Dean'){
       $type_designation = 'Dean';
}elseif($type == 'Head'){
        $type_designation = 'Head';
}
$query = "
...
if(mysqli_num_rows($user_text)>0){
       ...
       $spreadsheet->getActiveSheet()
                     ->setCellValue('A5', "      I, $name $type_designation of the $dept_name commit to deliver and agree to be rated on the attainment of the following targets in accordance with the indicated measures for the period $start_of_term to $end_of_term");
}
...
//imidiate superviser
$spreadsheet->getActiveSheet()
                     ->setCellValue('A19',$supervisor)
                     ->setCellValue('H19',$supervisor)
                     ->setCellValue('E48',$supervisor)
                     ->setCellValue('J48',$supervisor);
// $query = " SELECT 
//        f.`firstname`,
//        f.lastname,
//        f.middlename,
